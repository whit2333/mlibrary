...
vector<aopt> goptions::getArgs(string name);
{
	vector<aopt> options;
	map<string, int> count;
	for(map<string, aopt>::iterator itm = optMap.begin();itm != optMap.end(); itm++) {
...
map<string, string> goptions::getOptMap()
{
	...
	for(map<string, aopt>::iterator it = optMap.begin(); it != optMap.end(); it++)
	{
		...
		if(it->second.type == 0) optmap[key] = to_string(it->second.arg);
...
{
	if(name.find("option gcard file") != string::npos) return;

	cout << "  > " << name << " set to: ";

	if(type) cout << args; // print the string
	else     cout << arg;  // print the number
	cout << endl;
