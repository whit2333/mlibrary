...
bool G4CadSetupFactory::buildLogical(GOptions* gopt, GVolume *s, map<string, G4Volume*> *g4s)
{
	...
	if(thisMaterial == nullptr) {
		...
		if(thisMaterial == nullptr) {
			...
			G4cerr << FATALERRORL << "  Fatal Error: options default material " << dmat << " not found. Exiting." << G4endl;
			exit(99);
		}
	}
